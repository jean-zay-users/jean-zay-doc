...
The recommended way to use Tensorboard on Jean Zay, is to use the `idrjup`
...
#### Mount a sshfs partition and read the logs locally

An alternative to the official solution is to mount a folder from Jean
Zay, where you store the logs generated by Tensorboard, and use
the Tesorboard web server locally to visualize them. Let's suppose that 
you store logs in `$SCRATH/tb_logs/`

1. Mount the folder locally: 
```
sshfs -o reconnect,ServerAliveInterval=15,ServerAliveCountMax=3,follow_symlinks \
  jeanzay:/gpfsscratch/rech/your/project/tb_logs/ $HOME/jeanzay/logs
```

2. Install tensorboard in your host machine:
```
pip install tensorboard
```

3. Launch tensorboard locally, in the host machine. 
```
tensorboard --logdir=$HOME/jeanzay/logs
```

A new browser window will open and show the progress of your training.
...
30 minutes of CPU time, which roughly translates to 11 hours for Tensorboard dev.
